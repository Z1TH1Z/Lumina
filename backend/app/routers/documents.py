...
async def upload_document(
    ...
    try:
        ...
        for txn_data in result.get("transactions", []):
            ...
            # Add to list for RAG indexing
            all_txns_for_index.append({
                "date": txn_date.strftime("%Y-%m-%d") if txn_date else "Unknown",
                "description": txn_data.get("description", ""),
                "merchant": txn_data.get("merchant", ""),
                "amount": txn_data.get("amount", 0),
                "category": cat_value
        ...
        # Index for RAG (now including categorized transactions for context)
        await index_document(
            doc.id, 
            doc.raw_text, 
            {"filename": actual_filename},
            transactions=all_txns_for_index,
            user_id=current_user.id,
        )
