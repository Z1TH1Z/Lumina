...
def _extract_merchant(description: str) -> str:
    """Extract and normalize a merchant name from a transaction description."""
    # Strip common bank narration prefixes that carry no merchant signal
    desc = re.sub(
        r'^(POS|DEBIT|CREDIT|ACH|TRANSFER|CHECK|ATM|WIRE|NEFT|IMPS|UPI|RTGS|NACH|EMI)\s*[/\-]?\s*',
        '', description, flags=re.IGNORECASE,
    )
    # Remove long numeric reference codes (UPI IDs, txn refs, 6+ digit numbers)
    desc = re.sub(r'\b\d{6,}\b', '', desc)
    desc = re.sub(r'\b[A-Z0-9]{16,}\b', '', desc)
    # Take first 3 meaningful words as merchant name
    words = [w for w in desc.split() if len(w) > 1][:3]
    raw = " ".join(words) if words else description

    # Normalize to canonical merchant name using the shared map
    try:
        from app.services.categorization import normalize_merchant
